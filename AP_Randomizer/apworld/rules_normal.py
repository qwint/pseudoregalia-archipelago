    ...
    def __init__(self, world) -> None:
        ...
        region_clauses = {
            ...
            "Tower Remains -> The Great Door": lambda state:
                self.can_attack(state) and self.get_clings(state, 6) and self.kick_or_plunge(state, 1),
            ...
            "Castle Main -> Castle => Theatre Pillar": lambda state:
                self.get_clings(state, 6) and self.kick_or_plunge(state, 1)
                or self.kick_or_plunge(state, 2),
            "Castle Main -> Castle Spiral Climb": lambda state:
                self.get_kicks(state, 2)
                or self.get_clings(state, 6) and self.has_plunge(state),
            ...
            "Castle By Scythe Corridor -> Castle => Theatre (Front)": lambda state:
                self.get_clings(state, 6) and self.kick_or_plunge(state, 2),
            ...
            "Underbelly Main Upper -> Underbelly By Heliacal": lambda state:
                ...
                and (
                    ...
                    or self.get_clings(state, 6) and self.get_kicks(state, 2)),
            ...
            "Underbelly Hole -> Underbelly Main Lower": lambda state:
                ...
                and (
                    self.can_attack(state)
                    or self.can_slidejump(state) and self.get_clings(state, 6)
                    or self.can_slidejump(state) and self.get_kicks(state, 1)),
        ...
        location_clauses = {
            ...
            "Empty Bailey - Cheese Bell": lambda state:  # TODO consider to/from center steeple
                ...
                or self.get_clings(state, 6) and self.kick_or_plunge(state, 2),
            ...
            "Dilapidated Dungeon - Dark Orbs": lambda state:
                self.get_clings(state, 6) and self.can_bounce(state)
                or self.get_clings(state, 6) and self.kick_or_plunge(state, 3)
            ...
            "Dilapidated Dungeon - Rafters": lambda state:
                ...
                or self.knows_obscure(state) and self.can_bounce(state) and self.get_clings(state, 6),
            ...
            "Castle Sansa - Floater In Courtyard": lambda state:
                ...
                or self.knows_obscure(state) and self.get_clings(state, 6) and self.get_kicks(state, 1),
            ...
            "Castle Sansa - Alcove Near Scythe Corridor": lambda state:
                self.get_clings(state, 6) and self.get_kicks(state, 1) and self.has_plunge(state)
                or self.kick_or_plunge(state, 4),
            ...
            "Listless Library - Time Trial": lambda state:
                self.has_breaker(state) and self.has_plunge(state) and self.get_clings(state, 6),
        ...
        if self.world.options.game_version == MAP_PATCH:
            ...
            region_clauses["Dungeon Strong Eyes -> Dungeon => Castle"] = (lambda state:
                ...
                and (
                    ...
                    or self.can_attack(state) and self.get_clings(state, 6)))
            ...
            location_clauses["Dilapidated Dungeon - Strong Eyes"] = (lambda state:
                ...
                and (
                    self.get_clings(state, 6) and self.get_kicks(state, 1) and self.has_plunge(state)
                    or self.get_clings(state, 6) and self.get_kicks(state, 3)))
